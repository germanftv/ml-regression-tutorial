...
def run_env_check(raise_exc=False):
    ...
    def parse_package_version(line):
        """Parse package version from requirement string."""
        try:
            if '>=' in line:
        ...
        # Clean package name (remove conda channels)
        pkg = pkg.strip()
        if '::' in pkg:
            pkg = pkg.split('::')[-1]
    ...
    for line in env['dependencies']:
        if isinstance(line, dict):
            ...
            if 'pip' in line:
                for pip_pkg in line['pip']:
                    pkg, version = parse_package_version(pip_pkg)
                    requirements[pkg] = version
        elif isinstance(line, str):
            # Handle conda packages
            pkg, version = parse_package_version(line)
            if '-' not in pkg:  # Skip packages with dashes in name if needed
                requirements[pkg] = version
    ...
    for pkg, req_version in requirements.items():
        try:
            # Special case for scikit-learn: package name is 'scikit-learn' but import name is 'sklearn'
